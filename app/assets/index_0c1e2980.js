...
`;return u+=`
*${r.length} registered*`,{message:u}}case"show":{const r=t[1];if(!r)return{error:!0,message:"❌ Template id required: `::template show <ID>`"};const u=nn(r);if(!u)return{error:!0,message:`❌ Not registered: \`${r}\``};let o=`🧩 **${u.id}**
...
`,{message:u}}}const Fu=Object.freeze(Object.defineProperty({__proto__:null,default:Du},Symbol.toStringTag,{value:"Module"}));function Pu(){return`
  ...
  `}function so(){console.log("[ResethashRoute] Initialized");const e=document.getElementById("resethash-form"),t=e.querySelector('button[type="submit"]');ln("toggleNewPassword","new-password","eyeIconNew"),ln("toggleConfirmPassword","confirm-password","eyeIconConfirm"),e?.addEventListener("submit",async n=>{n.preventDefault();const r=e.email.value.trim(),u=e.newPassword.value,o=e.confirmPassword.value,a=document.getElementById("new-password"),i=document.getElementById("confirm-password");if(!u){Ae(a,k.get("validation_new_password_required"));return}const s=Yn(u);if(s.length>0){Ae(a,Kn(s));return}if(!o){Ae(i,k.get("validation_new_password_required"));return}if(u!==o){Ae(i,k.get("validation_password_mismatch"));return}t.disabled=!0,t.textContent=k.get("loading_resetting_password");try{const c=Nt(u),f=await fe.doResetHash(r,c);f.ok?(typeof neodigmToast<"u"&&neodigmToast.q(k.get("success_password_reset"),"success"),b.publish(g.AUTH_RESETHASH_SUCCESS,{email:r})):(Ae(a,f.error||k.get("error_signin_failed")),t.disabled=!1,t.textContent=k.get("update_password").toUpperCase())}catch(c){console.error("[ResethashRoute] Reset password error:",c),Ae(a,c.message||k.get("error_network")),t.disabled=!1,t.textContent=k.get("update_password").toUpperCase()}})}function ln(e,t,n){const r=document.getElementById(e),u=document.getElementById(t),o=document.getElementById(n);!r||!u||!o||r.addEventListener("click",()=>{const a=u.type==="password";u.type=a?"text":"password",a?o.innerHTML=`
        <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path>
        <line x1="1" y1="1" x2="23" y2="23"></line>
      `:o.innerHTML=`
        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
...
`;class So extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._playing=!1,this._note="",this._timer=null}connectedCallback(){this.build(),this.sync(),this._timer=setInterval(()=>this.sync(),Ao)}disconnectedCallback(){clearInterval(this._timer),this._timer=null}get installed(){return typeof window.startVCR=="function"}get recording(){return!!window._isVCRRecording}get tape(){return window._vcrTimeline??[]}build(){if(!this.installed){this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.addEventListener("click",this.onClick)}onClick=t=>{const n=t.target?.closest?.("button")?.dataset?.act;n&&this.run(n)};opt(t){const n=this.shadowRoot.querySelector(`[data-opt="${t}"]`);return n?n.type==="checkbox"?n.checked:n.value:null}async run(t){try{switch(t){case"record":window.startVCR(),this._note="";break;case"stop":window.stopVCR({save:!1}),this._note=`stopped - ${this.tape.length} event(s) on tape`;break;case"save":window.saveVCR(),this._note="written to Downloads";break;case"load":{const n=await window.loadVCR();this._note=`loaded ${n??this.tape.length} event(s)`;break}case"play":{if(this._playing)return;this._playing=!0,this._note="",this.sync();const n=await window.playVCR({speed:Number(this.opt("speed")),dryRun:this.opt("dryRun"),skip:this.opt("skip")?["AUTH","LLM"]:[]});this._playing=!1,this._note=n.cancelled?`cancelled after ${n.played}`:`played ${n.played}, skipped ${n.skipped} in ${n.durationMs}ms`;break}case"cancel":window.stopPlayback(),this._playing=!1;break}}catch(n){console.error("[M5TVcrControls] action failed",t,n),this._playing=!1,this._note=n.message}this.sync()}sync(){if(!this.installed)return;const{length:t}=this.tape,n=this.recording,r=this._playing,u=n?"recording":r?"playing":t?"loaded":"idle",o={recording:"Recording",playing:"Replaying",loaded:"Tape ready",idle:"Idle"};this.shadowRoot.querySelector(".dot").dataset.state=u,this.shadowRoot.querySelector(".label").textContent=o[u],this.shadowRoot.querySelector(".count").textContent=t?`${t} event(s)`:"";const a={record:n||r,stop:!n,save:!t||n,load:n||r,play:!t||n||r,cancel:!r};for(const[c,f]of Object.entries(a))this.shadowRoot.querySelector(`[data-act="${c}"]`).disabled=f;const i=this.tape.filter(c=>To.test(c.message)).length,s=this.shadowRoot.querySelector(".note");this._note?(s.className="note",s.textContent=this._note):i&&!this.opt("skip")&&!this.opt("dryRun")?(s.className="note warn",s.textContent=`⚠ ${i} AUTH/ROUTE/LLM event(s) will really fire`):(s.className="note",s.textContent="")}}customElements.define("m5t-vcr-controls",So);const fn=50;function hn(){const e=window.AuthController?.getEntity?.();return e?.class?typeof e.class=="object"?e.class.value??0:At(e.class)?.value??0:0}kt({id:"ACCOUNT_SUMMARY",when:"the user asks about their own account, profile, role, or plan",render:()=>{const e=window.AuthController?.getEntity?.();return e?`
      ...
      <p>
        <a data-n55-prompt="What can I do with the ${Ee(e.class)} role?">what this role allows</a>
        &nbsp;·&nbsp;
        <a data-n55-prompt="How do I change my password?">change password</a>
      </p>
    `:"<p>No account is signed in.</p>"},transcript:()=>{const e=window.AuthController?.getEntity?.();return e?`Showed the user their account summary: ${e.email}, role ${e.class}, status ${e.status}. They can see it on screen.`:"Tried to show an account summary, but nobody is signed in."}});kt({id:"VCR_TIME_TRAVEL",when:"the user explicitly asks to record or replay this session with the event tape recorder, or to time-travel debug",render:()=>hn()<fn?"<p>Time-travel debugging needs an administrator role.</p>":`
      <h3>Time Travel</h3>
      <p>
    ...
    `,transcript:()=>hn()<fn?"The user asked to time-travel debug, but their role does not allow it. Nothing was shown.":"Showed the user the time-travel debugging panel: transport controls for the PubSub tape recorder (record, stop, save, load, play, cancel) with speed, dry-run, and skip-AUTH/LLM options. It is on screen and working - do not repeat the button list back to them."});kt({id:"DASHBOARD",when:"the user asks about their data, engagements, apps, activity, metrics, or anything else inside the application that the other views do not specifically cover - this is the general landing view",render:()=>{const e=window.AuthController?.getEntity?.(),t=e?.first?`, ${Ee(e.first)}`:"",n=[["Engagements","—"],["Active apps","—"],["Messages","—"]];return`
      <h3>Dashboard</h3>
      ...
      <div class="table-scroll">
        <table>
          <thead>
            <tr>${n.map(([r])=>`<th scope="col">${Ee(r)}</th>`).join("")}</tr>
          </thead>
          <tbody>
            <tr>${n.map(([,r])=>`<td>${Ee(r)}</td>`).join("")}</tr>
          </tbody>
        </table>
      </div>
...
`:`<br>
`:`
`};ce.text=function(e,t){return me(e[t].content)};ce.html_block=function(e,t){return e[t].content};ce.html_inline=function(e,t){return e[t].content};function Re(){this.rules=tt({},ce)}Re.prototype.renderAttrs=function(t){let n,r,u;if(!t.attrs)return"";for(u="",n=0,r=t.attrs.length;n<r;n++)u+=" "+me(t.attrs[n][0])+'="'+me(t.attrs[n][1])+'"';return u};Re.prototype.renderToken=function(t,n,r){const u=t[n];let o="";if(u.hidden)return"";u.block&&u.nesting!==-1&&n&&t[n-1].hidden&&(o+=`
