...
⚠️ Theme switching not yet implemented in UI.`})}static cmdAuth(e){if(!e)return{error:!0,message:"❌ Not authenticated"};let t="Unknown",n=0;if(typeof e.class=="string"){const r=Oe(e.class);r&&(t=r.desc,n=r.value)}else typeof e.class=="object"&&(t=e.class.desc||"Unknown",n=e.class.value||0);let o=`🔐 **Authentication Status**
...
`,{message:o}}}const Ft=Object.freeze(Object.defineProperty({__proto__:null,default:Ut},Symbol.toStringTag,{value:"Module"}));function Dt(){return`
  ...
  `}function jt(){console.log("[SignupRoute] Initialized");const a=document.getElementById("signupForm"),e=a.querySelector('button[type="submit"]');Ne("togglePassword","password","eyeIcon"),Ne("toggleConfirmPassword","confirmPassword","eyeIconConfirm"),document.querySelector('.auth-switch a[href="#/signin"]')?.addEventListener("click",n=>{n.preventDefault(),console.log("[SignupRoute] Sign in link clicked - navigating to signin"),i.publish(l.ROUTE_NAV,{route:"signin"})}),a.addEventListener("submit",async n=>{n.preventDefault();const o=document.getElementById("first").value.trim(),r=document.getElementById("last").value.trim(),s=document.getElementById("email").value.trim(),c=document.getElementById("company").value.trim(),p=document.getElementById("phone").value.trim(),m=document.getElementById("password").value,E=document.getElementById("confirmPassword").value,L=document.getElementById("first"),w=document.getElementById("last"),R=document.getElementById("email"),D=document.getElementById("password"),u=document.getElementById("confirmPassword");if(Re(o,L,_.get("validation_first_name_required"))&&Re(r,w,_.get("validation_last_name_required"))&&Yt(s,R)&&Wt(m,D)){if(m!==E){B(u,_.get("validation_password_mismatch"));return}e.disabled=!0,e.textContent=_.get("loading_creating_account"),await Kt(o,r,s,c,p,m)}})}function Ne(a,e,t){const n=document.getElementById(a),o=document.getElementById(e),r=document.getElementById(t);!n||!o||!r||n.addEventListener("click",()=>{const s=o.type==="password";o.type=s?"text":"password",s?r.innerHTML=`
  ...
  `}function Xt(){console.log("[VerfLinkRoute] Initialized");const a=7200,e=Date.now(),t=document.getElementById("countdown-display");function n(){const r=Date.now()-e,s=Math.floor(r/1e3),c=Math.max(0,a-s),p=Math.floor(c/3600),m=Math.floor(c%3600/60),E=c%60,L=`${String(p).padStart(2,"0")}:${String(m).padStart(2,"0")}:${String(E).padStart(2,"0")}`;t&&(t.textContent=L,c<600&&(t.style.color="#ef4444",t.style.textShadow=`
